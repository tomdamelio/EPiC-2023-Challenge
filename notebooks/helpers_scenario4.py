...
def split_videos_train_test(videos, splits):
    """Split subjects into train and test sets.
    ...
    Returns:
        splits: list of dictionaries with keys 'train' and 'test' and values as lists of video numbers.
    """
    def partition (list_in, n):
        random.shuffle(list_in)
        return [list_in[i::n] for i in range(n)]
    
    partitions  = partition(videos, 3)
    
    splits = []

    for i in partitions:
        train = [x for x in videos if x not in i]
        test = i
